...
uint32_t  DCD_EP_Tx ( USB_OTG_CORE_HANDLE *pdev, uint8_t epaddr, uint8_t   *pbuf,  uint32_t   buf_len){
  USB_OTG_EP *ep = &pdev->dev.in_ep[epaddr & 0x7f];
...
uint32_t  DCD_EP_Stall (USB_OTG_CORE_HANDLE *pdev, uint8_t   epnum){
	TWB_Debug_Print("STALL ENDPOINT");
	USB_OTG_EP *ep;
	if ((0x80 & epnum) == 0x80)
