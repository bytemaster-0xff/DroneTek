...
void __twb_i2c_completeJob(iOpResult_e result){
	...
	if (__twbI2CCurrentJob->Callback != 0x00){
		if (((uint32_t)__twbI2CCurrentJob->Callback & 0x08000000) != 0x08000000){
			TWB_Debug_PrintInt("Address: ",__twbI2CCurrentJob->Address);
			TWB_Debug_Print2Int("Bytes: ",__twbI2CCurrentJob->BytesSent, __twbI2CCurrentJob->BytesReceived);
			TWB_Debug_Print2Int("I2C Pointers", I2CQueueJobHead, I2CQueueJobTail);
		}
