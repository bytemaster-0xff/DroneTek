...
NiVekMessage *NiVekMessageScribe::getNextToSend()
{
	if (m_messageSlotHead == m_messageSlotTail)
		return 0;

	NiVekMessage *next = &m_messageSlots[m_messageSlotTail++];

	if (m_messageSlotTail == MESSAGE_MAX_COUNT)
		m_messageSlotTail = 0;
...
void NiVekMessageScribe::endMessage()
{
	m_messageSlotHead++;
