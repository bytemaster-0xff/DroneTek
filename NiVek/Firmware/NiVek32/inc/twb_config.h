...
typedef struct {
	...
	uint16_t pitch_rate_i_decay_factor; // 6
	uint16_t pitch_rate_max_i; // 8
	int16_t k_pitch_steady_p; // 10
	int16_t k_pitch_steady_i; // 12
	uint16_t pitch_steady_max_i; // 14


	int16_t k_roll_rate_p;  	// 16
	int16_t k_roll_rate_i;  	// 18
	int16_t k_roll_rate_d;  	// 20
	uint16_t roll_rate_i_decay_factor;  // 22
	uint16_t roll_rate_max_i; 	// 24
	int16_t k_roll_steady_p;   	// 26
	int16_t k_roll_steady_i;   	// 28
	uint16_t roll_steady_max_i; 	// 30


	int16_t k_heading_rate_p; // 32
	int16_t k_heading_rate_i; // 34
	int16_t k_heading_rate_d; // 36
	uint16_t heading_rate_i_decay_factor; //38
	uint16_t heading_rate_max_i; 	// 40
	int16_t k_heading_steady_p; 		// 42
	int16_t k_heading_steady_i; 		// 44
	uint16_t heading_steady_max_i; 	// 46


	int16_t k_altitude_rate_p; 		// 48
	int16_t k_altitude_rate_i; 		// 50
	int16_t k_altitude_rate_d; 		// 52
	uint16_t altitude_rate_i_decay_factor; // 54
	uint16_t altitude_rate_max_i; 	// 56
	int16_t k_altitude_steady_p; 		// 58
	int16_t k_altitude_steady_i; 		// 60
	uint16_t altitude_steady_max_i; 	// 62

	int16_t k_esc_front; 			// 64
	int16_t k_esc_left;  			// 66
	int16_t k_esc_right; 			// 68
	int16_t k_esc_rear;  			// 70

	uint8_t ESCUpdateRate; 			// 72

	uint8_t PitchStableBand; 		// 73 - Degrees where we do not apply and stability, in 1/10 of a degree
	uint8_t RollStableBand; 		// 74 - Degrees where we do not apply and stability, in 1/10 of a degree
	uint8_t HeadingStableBand; 		// 75 - Degrees where we do not apply and stability, in 1/10 of a degree
	uint8_t AltitudeStableBand; 	// 76 - CMs where don't apply any corrections, in 1/10 of a degree

	uint8_t PIDSampleRate; 			// 77

	int8_t PitchSensitivity; 		// 78  -10.0 - 10.0 as a scaled decimal
	int8_t RollSensitivity; 		// 79 -10.0 - 10.0 as a scaled decimal
	int8_t YawSensitivity; 			// 80 -10.0 - 10.0 as a scaled decimal

	uint8_t pitch_filter_d; 		// 81
	uint8_t roll_filter_d;  		// 82
	uint8_t heading_filter_d;  		// 83
	uint8_t altitude_filter_d;  	// 84
	FrameConfig_e FrameConfig; 		// 85

	uint8_t InitialThrottle; 		// 86 - 0 - 255 Throttle Percent
	uint8_t InitialAltitude; 		// 87 - 0 - 255 CM

	SampleRate_t AutoPilotSampleRate; 	// 88

	PitchFollowRollPID_e PitchFollowsRollPID; // 89 0x00/0xFF
...
typedef struct {
	...
	SampleRate_t StatusSendDataRate; /* 7 Values */
	SampleRate_t GPSSendDataRate;
	SampleRate_t BatterySendDataRate;
	SampleRate_t SensorSendDataRate;
	SampleRate_t TargetsSendDataRate;
	SampleRate_t MotorsSendDataRate;
	SampleRate_t AltitudeSendDataRate;

	Enabled_t ITG3200_Enabled; /* 5 Values */
	uint8_t ITG3200_DLPF_FS;
	uint8_t ITG3200_SMPLRT_DIV;
	FilterOptions_t ITG3200_FilterOption;
	SampleRate_t ITG3200_SampleRate;
	...
	SampleRate_t MPU60x0_GyroSampleRate;
	SampleRate_t MPU60x0_AccSampleRate;
	...
	SampleRate_t LSM303_ACC_SampleRate;
	...
	SampleRate_t LSM303_MAG_SampleRate;
	...
	SampleRate_t L3GD20_SampleRate;
	...
	SampleRate_t ADXL345_SampleRate;
