...
iOpResult_e TWB_GPIO_CtrlLoop_SetPitchRollPID(void){
	...
	PIDRoll->IDecayFactor = PIDConstantsConfig->roll_rate_i_decay_factor == 0 ? FP_NAN:  ((float)PIDConstantsConfig->roll_rate_i_decay_factor) / DECAY_FACTOR_SCALER;
	...
	if(PIDConstantsConfig->PitchFollowsRollPID == PitchDoesFollowRollPID){
		...
		PIDPitch->IDecayFactor = PIDRoll->IDecayFactor;
		...
		PIDPitch->IDecayFactor = PIDConstantsConfig->pitch_rate_i_decay_factor == 0 ? FP_NAN : ((float)PIDConstantsConfig->pitch_rate_i_decay_factor) / DECAY_FACTOR_SCALER;
...
iOpResult_e TWB_GPIO_CtrlLoop_SetAltitudePID(void){
	...
	PIDAltitude->IDecayFactor = PIDConstantsConfig->altitude_rate_i_decay_factor == 0 ? FP_NAN : ((float)PIDConstantsConfig->altitude_rate_i_decay_factor) / DECAY_FACTOR_SCALER;
...
iOpResult_e TWB_GPIO_CtrlLoop_SetYawPID(void){
	...
	PIDYaw->IDecayFactor = PIDConstantsConfig->heading_rate_i_decay_factor == 0 ? FP_NAN : ((float)PIDConstantsConfig->heading_rate_i_decay_factor) / DECAY_FACTOR_SCALER;
