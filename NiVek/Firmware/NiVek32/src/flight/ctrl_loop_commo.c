...
float __ctrlLoop_resolve_DFilter(uint8_t setting){
	switch(setting){
		case 0: return 0.0; /* 10Hz */
		case 1: return 0.3f; /* 10Hz */
		case 2: return 0.25f;  /* 15Hz */
		case 3: return 0.1f;  /* 20Hz */
		case 4: return 0.075;  /* 25Hz */
		case 5: return 0.05;  /* 30Hz */
		case 6: return 0.025;  /* 30Hz */
		case 7: return 0.01;  /* 30Hz */
	}
...
iOpResult_e __ctrlLoop_setPIDConstant(uint8_t *payload){
	...
	if((constId >= K_PITCH_RATE_P_ADDR) && (constId <= K_ROLL_STEADY_I_MAX_ADDR)){
		...
		switch(constId){
		  /* Start Altitude Settings */
			...
			case K_ALTITUDE_RATE_I_DECAY_FACTOR_ADDR: PIDConstantsConfig->altitude_rate_i_decay_factor = value; PIDAltitude->IDecayFactor =  value == 0 ? FP_NAN : (float)value / DECAY_FACTOR_SCALER;  break;
			...
			case K_YAW_RATE_I_DECAY_FACTOR_ADDR: PIDConstantsConfig->heading_rate_i_decay_factor = value; PIDYaw->IDecayFactor =  value == 0 ? FP_NAN : (float)value / DECAY_FACTOR_SCALER;  break;
	...
	else if(constId >= K_PITCH_SENSITIVITY && constId <= K_YAW_SENSITIVITY){
		...
		assert_succeed(TWB_SE_WriteU8(K_INITIAL_ALTITUDE, payload[2]));

		TWB_Debug_PrintInt("Set initial altitude (cm): ", payload[2]);
	}
