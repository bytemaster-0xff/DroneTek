...
uint8_t USBD_SetupStage(USB_OTG_CORE_HANDLE *pdev){
	...
	/*TWB_Debug_PrintHexString(req.bmRequest);
	TWB_Debug_Print(".");
	TWB_Debug_PrintHexString(req.bRequest);
	TWB_Debug_Print(".");
	TWB_Debug_PrintHexString((req.wValue >> 8) & 0xFF);
	TWB_Debug_PrintHexString(req.wValue & 0xFF);
	TWB_Debug_Print(".");
	TWB_Debug_PrintHexString((req.wIndex >> 8) & 0xFF);
	TWB_Debug_PrintHexString(req.wIndex & 0xFF);
	TWB_Debug_Print(".");
	TWB_Debug_PrintHexString((req.wLength >> 8) & 0xFF);
	TWB_Debug_PrintHexString(req.wLength & 0xFF);*/
	...
	switch ((req.bmRequest & 0x60) >> 5){
		case 0x00:
			switch (req.bmRequest & 0x1F){
				...
				default: TWB_Debug_Print(".err"); DCD_EP_Stall(pdev , req.bmRequest & 0x80); break;
			 }
			break;
		case 0x02: USBD_VendDevReq(pdev, &req); break;
		default:
			TWB_Debug_Print("OH NO!");
			break;
	}
...
uint8_t USBD_DataOutStage(USB_OTG_CORE_HANDLE *pdev , uint8_t epnum){
	...
	if(epnum == 0)  {
		...
		if ( pdev->dev.device_state == USB_OTG_EP0_DATA_OUT){
			if(ep->rem_data_len > ep->maxpacket){
				...
				if((pdev->dev.class_cb->EP0_RxReady != (void *)0)&& (pdev->dev.device_status == USB_OTG_CONFIGURED))
	...
	else if((pdev->dev.class_cb->DataOut != (void *)0) && (pdev->dev.device_status == USB_OTG_CONFIGURED)){
...
uint8_t USBD_DataInStage(USB_OTG_CORE_HANDLE *pdev , uint8_t epnum){
		...
		if ( pdev->dev.device_state == USB_OTG_EP0_DATA_IN){
			if(ep->rem_data_len > ep->maxpacket){
				...
				if((ep->total_data_len % ep->maxpacket == 0) && (ep->total_data_len >= ep->maxpacket) && (ep->total_data_len < ep->ctl_data_len )){
					...
					if((pdev->dev.class_cb->EP0_TxSent != (void *)0)&& (pdev->dev.device_status == USB_OTG_CONFIGURED))
	...
	else if((pdev->dev.class_cb->DataIn != (void *)0)&& (pdev->dev.device_status == USB_OTG_CONFIGURED)) {
		pdev->dev.class_cb->DataIn(pdev, epnum);
	}
	else
		TWB_Debug_Print("FROM OH SHITr\n");

	return USBD_OK;
}

uint8_t USBD_Reset(USB_OTG_CORE_HANDLE  *pdev){
	TWB_Debug_Print("USBD_Resetr\n");
