...
void D3DPanel::SetRotation(float pitch, float roll, float yaw){
	m_pitch = pitch;
	m_roll = roll;
	m_yaw= yaw;
}

void D3DPanel::SetLocation(float x, float y, float z){
	m_x = x;
	m_y = y;
	m_z = z;
}

void D3DPanel::SetScaling(float scaling){
	m_scaling = scaling;
}
...
void D3DPanel::Render()
{
	...
	XMMATRIX rotation = XMMatrixRotationRollPitchYaw(m_pitch, m_roll, m_yaw);
	XMMATRIX translation = XMMatrixTranslation(m_x, m_y, m_z);

	XMMATRIX scale = XMMatrixScaling(1, 1, m_scaling) * rotation;
...
void D3DPanel::LoadModels(){
	VSD3DStarter::Mesh::LoadFromFile(m_graphics, L"BuildCrew.cmo", L"", L"Folder", m_meshModels);
...
void D3DPanel::CreateDeviceResources()
{
	...
	m_graphics.Initialize(m_d3dDevice.Get(), m_d3dContext.Get(), D3D_FEATURE_LEVEL_11_1);
